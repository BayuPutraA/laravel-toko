              ...
              <div class="grid grid-cols-1 xl:gap-4 my-4">                    
                  <div class="flex flex-col gap-6 bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
                    <div class="flex justify-between items-center">
                      <h3 class="text-xl leading-none font-bold text-gray-900">Daftar Staff</h3>
                      <button id="openModal" onClick="changeAction('/tambah-staff')" class="ml-5 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3">Tambah Staff</button>
                    </div>
                          ...
                          <tbody class="divide-y divide-gray-100">
                            @foreach($data as $d)
                              <tr class="text-gray-500">
                                ...
                                <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left">{{ $d->jenis_kelamin }}</th>                                  
                                <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left">{{ $d->username }}</th>
                                <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left flex gap-2">
                                  ...
                                  data-jenis_kelamin="{{ $d->jenis_kelamin }}" data-username="{{ $d->username }}" 
                                  data-password="{{ $d->password }}" onClick="changeAction('/edit-staff', {{ $d->id_staff }})">Edit</a> | 
                                  <a href="/hapus-staff/{{ $d->id_staff }}" class="text-red-400">Delete</a>
                                </th>
                              </tr>
                            @endforeach
                          </tbody>
  ...
  <div class="hidden py-12 bg-gray-700 bg-opacity-75 transition duration-150 ease-in-out absolute top-0 right-0 bottom-0 left-0 z-50" id="modal">
      <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
          ...
          <form id="my-form" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
              <input id="id_staff" name="id_staff" type="hidden" />           
              <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Input Staff</h1>
              ...
              <input id="username" name="username" class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="Masukkan Username" />
  ...
  <script type="text/javascript">
    $(document).ready(function () {  
      ...
      $('.edit-button').click((e) => {
        ...
        $('#jenis_kelamin').val(e.currentTarget.dataset.jenis_kelamin);
        $('#username').val(e.currentTarget.dataset.username);
        $('#password').val(e.currentTarget.dataset.password);
        $('#repassword').val(e.currentTarget.dataset.password);
      })

    });

    function changeAction(action, id_staff = null){
