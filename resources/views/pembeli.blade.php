              ...
              <div class="grid grid-cols-1 xl:gap-4 my-4">                    
                  <div class="flex flex-col gap-6 bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
                    <div class="flex justify-between items-center">
                      ...
                      <button id="openModal" onClick="changeAction('/tambah-pembeli')" class="ml-5 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3">Tambah Pembeli</button>
                    </div>
                          ...
                          <tbody class="divide-y divide-gray-100">
                            @foreach($data as $d)
                              <tr class="text-gray-500">
                                ...
                                <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left">{{ $d->ttl }}</th>
                                ...
                                <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left flex gap-2">
                                  <a href="#!" id="openModalEdit" class="text-green-500 edit-button" data-nama="{{ $d->nama }}" data-ttl="{{ $d->ttl }}" data-jenis_kelamin="{{ $d->jenis_kelamin }}"
                                  data-alamat="{{ $d->alamat }}" data-username="{{ $d->username }}" data-password="{{ $d->password }}" onClick="changeAction('/edit-pembeli', {{ $d->id_pembeli }})">Edit</a> | 
                                  <a href="/hapus-pembeli/{{ $d->id_pembeli }}" class="text-red-400">Delete</a>
                                </th>
                              </tr>
                            @endforeach
                          </tbody>
  ...
  <script type="text/javascript">
    $(document).ready(function () {  
      ...
      $('.edit-button').click((e) => {
        ...
        $('#ttl').val(e.currentTarget.dataset.ttl);
    ...
    function changeAction(action, id_pembeli = null){
